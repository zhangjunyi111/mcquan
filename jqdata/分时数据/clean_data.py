...
def create_mysql_engine() -> any:
    db_conn = create_engine(
        'mysql+pymysql://root:HONGhong1225@123.56.254.64/mcquant')
    return db_conn
...
def read_dipei_write_gaopei():
    ...
    db_conn = create_engine(
        'mysql+pymysql://root:HONGhong1225@123.56.254.64/mcquant', echo=True)
    ...
    for date in date_range:
        ...
        ztb_stock = pd.read_sql(
            f"select mc_code from ztb_stock where DATE_FORMAT("
            f"STR_TO_DATE("
            f"trade_date,"
            f"'%%Y-%%m-%%d %%H:%%i:%%s'),'%%Y-%%m-%%d') = "
            f"'{date_str_}'", db_conn)
        ...
        for code in codes:
            ...
            df = pd.read_sql(
                f"select * from fsdata_copy where "
